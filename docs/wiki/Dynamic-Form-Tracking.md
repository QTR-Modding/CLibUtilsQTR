...
5. If your plugin uses DFT's player active-effect restoration, read `GetPendingActiveEffects()` after `ReceiveData()`. Each record identifies the base (`baseFormid`), saved derivative (`dynamicFormid`), optional custom ID (`custom_id`), and elapsed seconds (`elapsed`). Recreate the required assignments and their properties even when no item remains in inventory. Then call `ApplyMissingActiveEffects()`. It restores missing tracked effects on the player; it is not a general NPC effect-restoration API.
