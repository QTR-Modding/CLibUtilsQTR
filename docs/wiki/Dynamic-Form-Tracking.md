...
5. If your plugin uses DFT's player active-effect restoration, read `GetPendingActiveEffects()` after `ReceiveData()`. Each record identifies the base (`baseFormid`), saved derivative (`dynamicFormid`), optional custom ID (`custom_id`), and elapsed seconds (`elapsed`). Recreate the required assignments and their properties even when no item remains in inventory. Then call `ApplyMissingActiveEffects()`. For newly written saves, each pending record also contains `effect`: the saved effect-list index, magic-effect FormID, elapsed time, total duration, the item's unscaled magnitude at save time, and the caster's FormID. It restores only those recorded definitions, leaving existing matching effects alone. Finding or restoring an effect marks its derivative active in DFT, so later saves retain the effect. The effect target remains the player; the caster may be another actor, including one that applied a poison. It is not a general NPC target effect-restoration API.

If a pending effect has no custom ID and its saved derivative must be replaced, call `RecreatePendingEffectForm(savedID)`. It obtains an unassigned replacement from the saved base and remaps all pending effects for that derivative. The result is the replacement FormID, or `0` on failure. Reapply your properties to that form before `ApplyMissingActiveEffects()`. Call it once per saved derivative, not once per effect.

If your plugin already created the appropriate replacement while restoring its inventory or other uses, call `RemapPendingActiveEffects(savedID, replacementID)` instead. The replacement must be owned by this tracker and belong to the same base. A false return leaves pending records unchanged. Both operations reject custom-ID effects; those use their restored custom assignment. Neither operation applies effects to the player.
...
The serializer retains the existing AoT/Containerize form-bank layout and appends a versioned list of individual active effects. This list records each effect's derivative FormID, effect-list index, magic-effect FormID, elapsed time, total duration, the item's unscaled magnitude at save time, and the caster's FormID. Restored effects retain their saved elapsed time and total duration, even if the item's configured duration has changed. The saved input magnitude is supplied before the engine applies scaling and compares competing effects. Current perks and other scaling still apply. Later saves read the item's then-current magnitude; DFT does not retain the original consumption inputs across subsequent saves. The caster ID is resolved through SKSE when loading. Effects with an unavailable caster are skipped instead of assigning the player as caster. Dispelled effects and effects whose duration has elapsed are excluded. An effect whose index or magic-effect FormID no longer matches the restored item is skipped. Older records without this extension remain readable, but only support best-effort restoration from their item-level elapsed time; already-expired effect definitions are skipped. Those legacy records have no caster information and retain the player-caster fallback. Arbitrary properties applied by your plugin are not serialized by this record. It uses the existing native binary representation, not a portable interchange format.
