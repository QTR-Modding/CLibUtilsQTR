...
If a pending effect has no custom ID and its saved derivative must be replaced, create one replacement for that saved derivative, apply your properties, and call `RemapPendingActiveEffects(savedID, replacementID)` before `ApplyMissingActiveEffects()`. It updates all pending effects for that saved derivative. Do this once per saved derivative, not once per effect. The replacement must be owned by this tracker and belong to the same base. A false return leaves the pending records unchanged. Custom-ID effects use their restored custom assignment instead and are not remapped by this operation.
